...
public class MovingPlatformEditor : Editor
{
    ...
    private Vector2[] handlePoints = new Vector2[0];
    ...
    void DrawPlatformPoints()
    {
        ...
        {
            handlePoints = new Vector2[source.points.Count];
        }
        ...
        for (int i = 0; i < source.points.Count; i++)
        {
            ...
            if (i > 0 && source.points[i] == Vector2.zero)
            {
                if (i > 1)
                {
                ...
                source.points[i] = new Vector2(firstPosX, firstPosY);
            }
    ...
    void SetPlatformStartPos()
    {
        ...
        {
            if (source.points[0] == Vector2.zero && source.transform.position != Vector3.zero)
                source.points[0] = source.transform.position;
            else if ((Vector2)source.transform.position != source.points[0] && !Application.isPlaying)
            {
