...
public class UnitAnimations : MonoBehaviour
{
    ...
    [SerializeField] protected AnimatorParamStateInfo animFacingRight;
    public bool FacingRight { set { if (anim && !forceIdle) anim.SetBool(animFacingRight.stringValue, value); } }
    ...
    void PlayRandomAnim(AnimatorParamStateInfo[] _anims)
    {
        if (_anims.Length < 1)
            return;
        var rand = Random.Range(0, _anims.Length - 1);
